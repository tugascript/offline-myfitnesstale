...
class UpdateWorkoutRecordEditor extends StatefulWidget {
  ...
  const UpdateWorkoutRecordEditor({
    super.key,
    required this.theme,
    required this.sizes,
    required this.units,
  ...
  @override
  State<UpdateWorkoutRecordEditor> createState() =>
      _UpdateWorkoutRecordEditorState();
}

class _UpdateWorkoutRecordEditorState extends State<UpdateWorkoutRecordEditor> {
  ...
  void _populateInitialData() {
    final record =
        context.read<WorkoutRecordCubit>().state.selectedWorkoutRecord;
    ...
    for (final setRecord in record.setRecords ?? []) {
      final setId = setRecord.workoutSetId;
      final iterationNumber = setRecord.setNumber;

      _iterationRestSecs[setId] ??= {};
      _iterationRestSecs[setId]![iterationNumber] =
          setRecord.totalRestSecs ?? 0;

      final setDto = widget.sets.firstWhere(
        (s) => s.id == setId,
        orElse: () => widget.sets.first,
      );

      final setDtos =
          setRecord.setExerciseRecords ?? <WorkoutSetExerciseRecordDto>[];
      for (final exRecord in setDtos) {
        int position = 0;
        if (setDto.exercises != null) {
          final idx = setDto.exercises!.indexWhere(
            (e) => e.id == exRecord.workoutSetExerciseId,
          );
          if (idx != -1) {
            position = setDto.exercises![idx].position;
          }
        }

        _setExercises[setId] ??= {};
        _setExercises[setId]![iterationNumber] ??= {};
        _setExercises[setId]![iterationNumber]![exRecord.workoutSetExerciseId] =
            CreateWorkoutSetExerciseRecordBatchDto(
          workoutSetExerciseId: exRecord.workoutSetExerciseId,
          exerciseId: exRecord.exerciseId,
          position: position,
          weight: exRecord.weightGrams,
          reps: exRecord.reps,
          difficulty: exRecord.difficulty,
        );
      }
    }
  }
  ...
  Future<void> _onSave() async {
    ...
    await context.read<WorkoutRecordCubit>().batchUpdateWorkoutRecord(
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<WorkoutRecordCubit, WorkoutRecordState>(
      listenWhen: (previous, next) =>
          previous.selectedWorkoutRecord != next.selectedWorkoutRecord,
      listener: (context, state) {
        ...
        if (state.selectedWorkoutRecord != null) {
          _populateInitialData();
          _startTimeController.text = _formatDateTime(widget.units, _startedAt);
          _endTimeController.text = _formatDateTime(widget.units, _completedAt);
          for (final set in widget.sets) {
